"""
Configuration validation for Nadir.
...
from nadir.config.schema import NadirConfig, CameraSource, FiducialBackend
...
def validate_config(config: NadirConfig, config_dir: Optional[Path] = None) -> None:
    """
    Perform cross-field and runtime validation on configuration.

    Args:
        config: NadirConfig instance to validate.
...
def _validate_camera(config: NadirConfig, config_dir: Optional[Path]) -> list[str]:
    """Validate camera configuration."""
    errors: list[str] = []
...
def _validate_fiducials(config: NadirConfig) -> list[str]:
...
def _validate_mavlink(config: NadirConfig) -> list[str]:
    """Validate MAVLink configuration."""
    errors: list[str] = []
...
def _validate_control(config: NadirConfig) -> list[str]:
    """Validate control configuration."""
    errors: list[str] = []
...
def _validate_landability(config: NadirConfig) -> list[str]:
    """Validate landability configuration."""
    errors: list[str] = []

    from nadir.config.schema import LandabilityMethod
